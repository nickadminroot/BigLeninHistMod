#!/usr/bin/env python3
"""
Windows-specific HOI4 smoke test with CreamAPI support.
For cross-platform use, prefer hoi4-smoke.py instead.

Usage:
    python hoi4-smoke-windows.py
...
Environment variables (all optional — auto-detected):
    ...
    HOI4_SMOKE_CREAM_UNLOCKALL - CreamAPI unlock (default: 1)
"""
...
HOI4_SOURCE_ROOTS = (
    "common", "events", "history", "localisation",
    "interface", "map", "gfx", "music", "sound",
)
...
def parse_duration(value: str) -> float:
    ...
    return amount * factors[factors.get(suffix, 1)]
...
def seed_dlc_state(normal_data: Path, game_data: Path, dlc_load_file: Path) -> None:
    ...
    dlc_load_file.write_text(
        json.dumps({"enabled_mods": [f"mod/{MOD_NAME}.mod"], "disabled_dlcs": disabled_dlcs}, indent=2) + "\n"
    )
...
def run_hoi4(hoi4_exe: Path, hoi4_dir: Path, crash_dir: Path, smoke_tag: str, timeout_seconds: float, launch_log: Path) -> int:
    command = [str(hoi4_exe), "-gdpr-compliant", "-debug_mode", "-debug", f"-start_tag={smoke_tag}", f"--crashdir={crash_dir}"]
    creationflags = getattr(subprocess, "CREATE_NEW_PROCESS_GROUP", 0)
    with launch_log.open("w", errors="replace") as log:
        proc = subprocess.Popen(command, cwd=hoi4_dir, env=os.environ.copy(), stdout=log, stderr=subprocess.STDOUT, creationflags=creationflags, text=True)
        try:
...
# Reuse log parsing from hoi4-smoke.py (simplified for Windows)
def find_entry_source(entry: str) -> Source:
    ...
    for pattern in [
        ...
        if match:
            ...
            path = groups[0].strip().replace("\\", "/").lstrip("./")
            line = None
...
def classify_errors(error_log: Path, include_pattern: str | None) -> tuple[list[str], list[str]]:
...
def main() -> int:
    ...
    if hoi4_dir is None:
        die("Could not find HOI4. Set HOI4_DIR environment variable.")

    pdx_user = find_pdx_user_dir()
    if pdx_user is None:
        die("Could not find Paradox user data. Set PDX_USER_DIR environment variable.")
    ...
    if not descriptor.is_file():
        for candidate in Path("..").glob("*/descriptor.mod"):
            if candidate.is_file():
                ...
                break
    if not descriptor.is_file():
        die(f"descriptor.mod not found")

    if not hoi4_exe.is_file():
        die(f"hoi4.exe not found: {hoi4_exe}")
    ...
    try:
        ...
        with backed_up_file(dlc_load_file), backed_up_file(mod_metadata_file), backed_up_file(cream_api_ini):
            ...
            seed_dlc_state(game_data_dir, game_data_dir, dlc_load_file)
            set_cream_unlockall(cream_api_ini, cream_unlockall)

            print(f"hoi4-smoke-windows: start timeout={smoke_timeout} tag={smoke_tag}", flush=True)
            print(f"hoi4-smoke-windows: data {game_data_dir}", flush=True)

            run_status = run_hoi4(hoi4_exe, hoi4_dir, crash_dir, smoke_tag, parse_duration(smoke_timeout), launch_log)

        if run_status not in (0, 124):
            print(f"hoi4-smoke-windows: HOI4 exited with status {run_status}")

        if not error_log.is_file():
            die(f"HOI4 did not create error.log")
        if previous_error_log_mtime is not None and error_log.stat().st_mtime <= previous_error_log_mtime:
            die(f"HOI4 did not update error.log")
        ...
        if entries:
            keep_data = True
            sources = [find_entry_source(e).path for e in entries]
            by_source = Counter(s for s in sources if s is not None)
            print(f"hoi4-smoke-windows: FAIL — {len(entries)} error entries", file=sys.stderr)
            for source, count in by_source.most_common(10):
                print(f"  {count:4d}  {source}", file=sys.stderr)
            return 1

        if keep_data:
            print(f"hoi4-smoke-windows: PASS; retained: {smoke_root}")
        else:
