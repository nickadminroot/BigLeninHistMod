...
6. When adding a focus that needs a new custom icon, keep a valid fallback `icon` in the focus and create `icon-manifests/focus/<focus_id>.json` with `scripts/icon-manifest.py new`. Do not generate images, edit the shared generated GFX, or switch to the custom sprite during normal focus implementation. Follow [GENERATING_ICONS.md](GENERATING_ICONS.md), including its parallel-branch integration workflow.
